...
# From the main list of genomes and what is to be included from read recruitment, make a tab delimited file with 271 representatives
genomes_to_recruit_271 <- read.delim("~/path/to/working/directory/global_read_recruitment/genomes_to_recruit.txt", stringsAsFactors = FALSE)
...
genomes_to_recruit_271_with_paths <- left_join(genomes_to_recruit_271, paths_genomes_all_genomes, by = c("accession"))

write.csv(genomes_to_recruit_271_with_paths, file = "~/path/to/working/directory/global_read_recruitment/genomes_to_recruit_271_with_paths.csv")

        # Copied all of the 271 genome files into a folder for read recruitment:
        #      ~/path/to/working/directory/genomes_271 with command "copy_command_genomes_271.txt"
...
for i in genomes_271/RENAME/*fa
do
    genome_name=`basename $i | awk 'BEGIN{FS=".fa"}{print $1}'`
    for c in `grep '>' genomes_271/RENAME/$genome_name.fa`
    do
...
cat genomes_271/RENAME/*.fa > SAR11_mapping/SAR11.fa
...
# K-means clustering
# Information for running this is all included in the clustering directory 
